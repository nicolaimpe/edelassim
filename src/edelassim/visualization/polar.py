...
def set_polarplot(ax: Axes, alt_max: int = 0, alt_min: int = 4800) -> Axes:
...
def plot_snowline_polarplot(
    snowline_per_aspects: np.ndarray, ax: Axes, label: str | None = None, color: str | None = None
) -> None:
...
def plot_ensemble_snowline_polarplot(
    ...
    ax.fill_between(theta, r_lower, r_upper, label=label, color=color, alpha=0.3)
    return ax
